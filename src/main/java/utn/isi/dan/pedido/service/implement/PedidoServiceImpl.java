...
/*			detallesPedidos.stream().forEach(detalle ->  
	...
	public void confirmarPedido(Pedido p) {
		...
		boolean stockDisponible = p.getDetalle().stream()
	...
	public Optional<DetallePedido> buscarDetalle(Integer idPedido, Integer idDetalle) {
		...
		if (pedido.isPresent()) {
			...
			Optional<DetallePedido> det = pedido.get().getDetalle().stream().filter(dp -> dp.getId().equals(idDetalle))
					.findFirst();
			...
			if (det.isPresent()) {
				...
				throw new NotFoundException(
						"Detalle pedido inexistente. IdPedido: " + idPedido + " IdDetalle: " + idDetalle);
	...
	public DetallePedido actualizarDetalle(Integer idPedido, DetallePedido detalle) {
		...
		if (pedido.isPresent()) {
			...
			Optional<DetallePedido> det = pedido.get().getDetalle().stream()
					.filter(dp -> dp.getId().equals(detalle.getId())).findFirst();
			...
			if (det.isPresent()) {
				...
				throw new NotFoundException(
						"Detalle pedido inexistente. IdPedido: " + idPedido + " IdDetalle: " + detalle.getId());
